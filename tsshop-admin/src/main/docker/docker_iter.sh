...
#docker_iter.sh aquarius-api 9023:9023 prod
projectName=$1
...
docker run -di --name "$projectName" --net=common_net -v "$docker_logs" -v /etc/localtime:/etc/localtime:ro --restart=always -p "$ports" "$imageNameNew"
# shellcheck disable=SC2181
if [ $? == 0 ]; then
  echo "新容器启动成功"
  ...
  if [ "$containerIdNew" != "" ]; then
    ...
    # 启动新容器
    # --net=common_net --restart=always
    docker run -di --name "$projectName" --net=common_net -v "$docker_logs" -v /etc/localtime:/etc/localtime:ro --restart=always -p "$ports" "$imageNameOld"
    if [ $? == 0 ]; then
      echo "部署回滚成功"
    else
      echo "部署回滚失败"
